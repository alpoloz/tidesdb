...
The filter is built as SSTables are written. Every key is hashed into a bitset, and the bitset is stored inside the SSTable itself, appended after the data blocks. A small footer at the end of the file records the Bloom offset and length so the filter can be found quickly during loads. On reads, the lookup checks the Bloom filter first; if the filter says "no," the table is skipped entirely. If it says "maybe," the search continues into the block index and data blocks.
...
Together, embedded Bloom filters and blocks reduce read amplification in two directions: filters prune whole tables, and block indices narrow the search within a table. The result is a format that stays simple but scales better as more SSTables accumulate.
